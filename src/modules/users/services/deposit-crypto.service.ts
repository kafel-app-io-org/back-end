...
export class DepositCryptoService {
  ...
  private readonly USDT_CONTRACT_ADDRESS: string;
  private readonly USDC_CONTRACT_ADDRESS: string;
  private readonly usdtWalletAddress: string;
  private readonly usdcWalletAddress: string;
  ...
  async verifyErc20Transfer(
    ...
    if (verifyPaymentDto.token === 'USDT') {
      contractAddress = this.USDT_CONTRACT_ADDRESS;
      walletAddress = this.usdtWalletAddress;
    } else {
      contractAddress = this.USDC_CONTRACT_ADDRESS;
    ...
    for (const log of txReceipt.logs) {
      if (
        ...
        const to = '0x' + log.topics[2].slice(26); // last 40 chars
        const amount = BigInt(log.data);
        // const amount = Number(formatUnits(rawAmount, 6));
        // const amount = ethers.BigNumber.from(log.data);

        const decimals = 4; // USDT and USDC
        const amountFormatted = parseInt(ethers.formatUnits(amount, decimals));
        if (
          to.toLowerCase() === walletAddress.toLowerCase() /*&&
          amountFormatted >= verifyPaymentDto.expectedAmount*/
        ) {
          return { success: true, amount: amountFormatted };
        }
      }
    }
  ...
  async create(
    ...
    const isPreviousRecord = await this.depositRepository.findOne({
      where: { intent_id: verifyPaymentDto.txHash },
    });
    if (isPreviousRecord) {
    ...
    const isSuccessTransfer = await this.verifyErc20Transfer(verifyPaymentDto);
  ...
  async checkPendingDeposits() {
    ...
    try {
      for (const deposit of pendingDeposits) {
        ...
        const isSuccessTransfer = await this.verifyErc20Transfer(
          verifyPaymentDto,
        );
        if (isSuccessTransfer.success) {
          await this.saveDepositTransaction(deposit, queryRunner.manager);
